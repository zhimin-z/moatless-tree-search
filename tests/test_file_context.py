...
def test_contextfile_flow_verification():
    ...
    # Second change
    context_file2 = ContextFile(repo=repo, file_path='test_file.txt', initial_patch=context_file1.patch)
    ...
    # Third change
    context_file3 = ContextFile(repo=repo, file_path='test_file.txt', initial_patch=context_file2.generate_full_patch())
...
def test_generate_full_patch():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for file_name, content in [('file1.py', 'original content 1\n'), ('file2.py', 'original content 2\n')]:
        ...
        for file_name, expected_content in [('file1.py', 'modified content 1\n'), ('file2.py', 'modified content 2\n')]:
            file_path = os.path.join(temp_dir, file_name)
            with open(file_path, 'r') as f:
                content = f.read()
                print(f"{file_name}:\n{content}\n")
                assert content == expected_content, f"Content of {file_name} does not match expected content after applying patch"
